...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'adi-digital-ultimate';
...
  { id: 21, nameBn: "আবহাওয়া খবর", nameEn: "Weather", icon: CloudSun, color: "#0ea5e9", link: "https://weather.com/" },
...
    if (user) {
      try {
        await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'data'), newData, { merge: true });
...
    if (user) {
      await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'profile', 'data'), updatedData, { merge: true });
    }
  };
...
      const matchSearch = (isBn ? s.nameBn : s.nameEn).toLowerCase().includes(search.toLowerCase());
      return matchSearch;
    });
  }, [search, isBn]);
...
        <form onSubmit={handleStart} className="w-full max-w-sm p-12 bg-slate-900/40 border border-slate-800 rounded-[3.5rem] backdrop-blur-3xl text-center shadow-2xl relative z-10 scale-in">
          <ADILogo size={130} />
...
            required
          />
          <button type="submit" className="w-full py-6 bg-gradient-to-br from-yellow-600 to-yellow-700 text-white rounded-2xl font-black flex items-center justify-center gap-3 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-yellow-900/20">
            {isBn ? "প্রবেশ ও সেভ করুন" : "Save & Enter"} <LogIn size={20} />
          </button>
          <p className="text-[10px] mt-6 text-slate-600 uppercase font-black tracking-widest flex items-center justify-center gap-2">
            <Cloud size={10}/> Cloud Storage Enabled
          </p>
        </form>
      </div>
    );
...
      <main className="max-w-5xl mx-auto p-4 md:p-6 pb-32 animate-fade-in">
        
        {/* Welcome Text */}
...
              className="w-full pl-16 pr-8 py-6 rounded-full border-2 border-slate-800 bg-slate-900 outline-none font-bold text-lg transition-all focus:border-yellow-500 shadow-2xl shadow-black"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
          <p className="text-[9px] text-slate-600 font-bold uppercase tracking-[0.2em]">
            User ID: {user?.uid} • <span className="text-yellow-600">Cloud Protection Active</span>
          </p>
        </div>
...
               className={`px-8 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-wider transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-yellow-600 text-white shadow-xl shadow-yellow-900/30' : 'bg-slate-800/40 text-slate-500 hover:text-slate-300'}`}
             >
               {isBn ? tab.bn : tab.en}
             </button>
           ))}
...
              <SectionHeader title={isBn ? "সিম অফার সেকশন" : "Sim Offers"} icon={Smartphone} colorClass="text-blue-500" />
              <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 px-2">
                {simOffers.map(item => (
                  <div key={item.id} onClick={() => { playPremiumSound(); setSelectedItem(item); }} className="flex-shrink-0 w-32 md:w-36 p-5 md:p-6 rounded-[2.5rem] md:rounded-[3rem] bg-slate-900 border-2 border-slate-800/60 hover:border-yellow-500/40 transition-all cursor-pointer group">
                    <div className="flex flex-col items-center gap-4">
                      <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center bg-slate-800/50" style={{color: item.color}}><item.icon size={28} /></div>
                      <p className="text-[10px] md:text-[11px] font-black text-center whitespace-nowrap">{isBn ? item.nameBn : item.nameEn}</p>
                    </div>
                  </div>
                ))}
              </div>
...
            <div className="animate-fade-in">
              <SectionHeader title={isBn ? "প্রিমিয়াম মুভি হাব" : "Premium Movie Hub"} icon={Film} colorClass="text-red-500" />
              <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 px-2">
                {movieHub.map(item => (
                  <div key={item.id} onClick={() => { playPremiumSound(); setSelectedItem(item); }} className="flex-shrink-0 w-32 md:w-36 p-5 md:p-6 rounded-[2.5rem] md:rounded-[3rem] bg-slate-900 border-2 border-slate-800/60 hover:border-red-500/40 transition-all cursor-pointer group shadow-lg shadow-black/40">
                    <div className="flex flex-col items-center gap-4">
                      <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center bg-slate-800/50" style={{color: item.color}}><item.icon size={28} /></div>
                      <p className="text-[10px] md:text-[11px] font-black text-center whitespace-nowrap">{isBn ? item.nameBn : item.nameEn}</p>
                    </div>
...
            <div className="animate-fade-in">
              <SectionHeader title={isBn ? "আর্নিং গেমস পোর্টাল" : "Earning Portal"} icon={Coins} colorClass="text-yellow-500" />
              <div className="flex gap-4 overflow-x-auto no-scrollbar pb-6 px-2">
...
        {activeTab === 'all' && (
          <div className="animate-fade-in">
            <SectionHeader title={isBn ? "ডিজিটাল সেবা সমূহ" : "Digital Services"} icon={LayoutGrid} colorClass="text-emerald-500" />
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6 px-2">
              {filteredServices.map(s => (
                <div key={s.id} onClick={() => { playPremiumSound(); setSelectedItem(s); }} className="p-5 md:p-7 rounded-[2.5rem] md:rounded-[3.5rem] bg-slate-900 border-2 border-slate-800/80 hover:border-yellow-500/50 transition-all cursor-pointer group flex flex-col items-center">
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-4 md:mb-5 bg-slate-800/40 transition-transform group-hover:scale-110" style={{color: s.color}}><s.icon size={26} /></div>
                  <h4 className="font-black text-[11px] md:text-[13px] leading-tight text-center px-1 h-8 flex items-center justify-center">{isBn ? s.nameBn : s.nameEn}</h4>
                  <div className="mt-2 w-6 h-1 bg-slate-800 rounded-full group-hover:bg-yellow-500 transition-colors"></div>
                </div>
              ))}
            </div>
          </div>
...
          <div className="relative w-full max-w-md p-10 rounded-[3rem] md:rounded-[4rem] bg-slate-900 border border-slate-800 shadow-2xl scale-in overflow-y-auto max-h-[90vh] no-scrollbar">
...
                {isBn ? "জরুরি সেবা (ফায়ার সার্ভিস/এম্বুলেন্স) এবং যাতায়াত (বাস/বিমান টিকিট) এখন যুক্ত করা হয়েছে।" : "Emergency services (Fire/Ambulance) and Transport (Bus/Air tickets) have been added."}
              </p>
              <div className="space-y-4">
                 <h4 className="text-white font-bold">{isBn ? "আপডেট নোট" : "Update Notes"}</h4>
                 <ul className="text-[10px] space-y-2 opacity-80 uppercase font-black tracking-widest">
                   <li className="flex items-center gap-2"><div className="w-1 h-1 bg-yellow-500 rounded-full"></div> Emergency: Fire, Ambulance Added</li>
                   <li className="flex items-center gap-2"><div className="w-1 h-1 bg-yellow-500 rounded-full"></div> Travel: Bus & Air Tickets</li>
                   <li className="flex items-center gap-2"><div className="w-1 h-1 bg-yellow-500 rounded-full"></div> Cloud: Firestore Auto-Sync</li>
                 </ul>
              </div>
...
          <div className="relative w-full max-w-sm p-10 md:p-12 rounded-[3rem] md:rounded-[4rem] bg-slate-900 border border-slate-800 shadow-2xl scale-in text-center">
            <div className="w-20 h-20 md:w-24 md:h-24 mx-auto rounded-[2rem] md:rounded-[2.5rem] bg-slate-800 flex items-center justify-center text-yellow-500 mb-8 shadow-inner animate-float"><selectedItem.icon size={44} /></div>
            <h3 className="text-2xl md:text-3xl font-black mb-4 italic">{isBn ? selectedItem.nameBn : selectedItem.nameEn}</h3>
            <p className="text-slate-500 text-xs md:text-sm mb-10 md:12 font-bold uppercase tracking-widest">{isBn ? 'এই সার্ভিসটি কি ওপেন করবেন?' : 'Open this service?'}</p>
            <div className="flex flex-col gap-4">
              <a href={selectedItem.link} target="_blank" rel="noopener noreferrer" onClick={() => setSelectedItem(null)} className="py-5 bg-gradient-to-r from-yellow-600 to-yellow-700 text-white rounded-[1.5rem] md:rounded-[2rem] font-black shadow-2xl shadow-yellow-900/40">{isBn ? 'প্রবেশ করুন' : 'Confirm'}</a>
              <button onClick={() => setSelectedItem(null)} className="py-2 text-slate-600 font-bold hover:text-slate-400 transition-all">{isBn ? 'ফিরে যান' : 'Go Back'}</button>
            </div>
          </div>
        </div>
      )}
...
        .scale-in { animation: scale-in 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
